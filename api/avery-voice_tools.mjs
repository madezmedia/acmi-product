/**
 * Avery Voice Tools — VAPI → Composio Webhook Handler
 * Real estate acquisition agent: Gmail, Calendar, Tasks, Maps, Search
 */
...
// Sent messaging — direct REST
const SENT_API_BASE = 'https://api.sent.dm/v3';
const SENT_API_KEY = process.env.SENT_API_KEY || '';

const TOOL_MAP = {
  // Email — send/receive about properties, leads
  averySendEmail: { slug: 'GMAIL_SEND_EMAIL', transform: (a) => ({ recipient_email: a.recipient_email, subject: a.subject, body: a.body, is_html: false }) },
  averyReadEmail: { slug: 'GMAIL_FETCH_EMAILS', transform: (a) => ({ max_results: a.max_results || 5, query: a.query || '', include_spam_trash: false }) },
  
  // Calendar — schedule viewings, meetings
  ...
  averyCreateEvent: { slug: 'GOOGLECALENDAR_CREATE_EVENT', transform: (a) => ({ summary: a.summary, start_datetime: a.start_time, end_datetime: a.end_time, description: a.description || '' }) },
  
  // Tasks — manage follow-ups
  averyAddTask: { slug: 'GOOGLETASKS_INSERT_TASK', transform: (a) => ({ title: a.title, notes: a.notes || '', due: a.due || null }) },
  
  // Web Search — market research, property comps
  ...
  // Sent messaging
  averySendMessage: { slug: '__SENT__', transform: null },
  
  // Lead logging — existing pipeline
  averyLogLead: { slug: 'CUSTOM_API_CALL', transform: (a) => ({ endpoint: '/api/avery/tools', action: 'logAcquisitionLead', data: a }) },
};
...
function extractResponse(inner) {
  ...
  try {
    ...
    for (const r of results) {
      ...
      if (payload.answer) return payload.answer.substring(0, 500);
...
export async function handler(req, res) {
  ...
  if (req.method === 'GET') return res.status(200).json({ ok: true, service: 'avery-voice-tools', tools: Object.keys(TOOL_MAP) });
  if (req.method !== 'POST') return res.status(405).json({ error: 'method_not_allowed' });

  try {
    const { name, arguments: args = {}, toolCallId } = req.body;
    const toolDef = TOOL_MAP[name];
    if (!toolDef) return res.status(200).json({ toolCallId, result: { content: [{ type: 'text', text: `Unknown tool. Available: ${Object.keys(TOOL_MAP).join(', ')}` }] } });
